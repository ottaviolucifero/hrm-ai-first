...
SELECT RANDOM_UUID(), user_account.id, user_account.tenant_id, 'TENANT_ADMIN', TRUE, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP
...
SELECT RANDOM_UUID(), user_account.tenant_id, user_account.id, role.id
FROM user_accounts user_account
JOIN user_types user_type
  ON user_type.id = user_account.user_type_id
JOIN roles role
