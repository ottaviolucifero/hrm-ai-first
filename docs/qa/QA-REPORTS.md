...
- Area verificata: `/login` (`login.component.html/.scss`), i18n `it/fr/en`, riuso `app-email-field` e `app-password-field`, coerenza TEMPLATE-06 senza modifiche funzionali
- Stati visuali verificati: idle (layout/gerarchia card), loading (CTA disabled + label submitting + `aria-busy`), errore credenziali (toast invariato), selettore lingua `it/fr/en`
- Comandi eseguiti: `cd frontend && npm.cmd run build` -> OK; `cd frontend && npm.cmd test` -> primo run KO per timeout intermittente (`master-data-admin.component.spec.ts`), rerun OK
- Esiti reali: build frontend OK; test frontend OK al rerun, 15 file di test passed, 85 test passed
...
- Limiti/note: validazione manuale live su `/login` (desktop/mobile, credenziali errate, cambio lingua) resta raccomandata in sessione browser dedicata
